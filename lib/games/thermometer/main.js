...
import { ThermoSVG } from "components/Boards/Thermometer";
...
export function Thermo(props) {
    ...
    return (
        <div id='view' className='w-full mx-auto'>
            <div ref={ref} className='max-h-80 max-w-80 grid justify-evenly grid-cols-4 bg-gray-50 shadow-md rounded-lg gap-4 p-4'>
                {data?.map((item, index) => (
...
export class Thermoc {
    ...
    makeData() {
        //create random data 
        let temps = Array.from({ length: 4 }, () => Math.floor(Math.random() * this.c))
